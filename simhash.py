...
class HashManager:
    ...
    def compare(self,simhash1,simhash2):
        combined = -1*~(simhash1^simhash2)
        count = 0
        while (combined): #this section of code found https://www.geeksforgeeks.org/count-set-bits-in-an-integer/, finds number of bits set to 1
            combined &= (combined - 1)
            count += 1
        return count/128
